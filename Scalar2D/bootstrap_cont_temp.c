...
void bootstrap(FILE *f, float m, int Nt, int Nx, float Ntm, float Nxm, char dir[100]){
    FILE *xene_mass ,*xene_spat, *xene_temp, *xene_density;
    unsigned long s; //serve per salvarsi il numero casuale enorme che produco
    int i=0,j,k,M=70,x,bin=1000,taglio=0;         /*M = numero di resampling, 
    ...
    char filemisure[900];
    sprintf(filemisure,"/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/%s/Bootstrap/misure_boot_mass_NO_RELAX(Ntm=%.3f,Nxm=%.3f,Nt=%d,Nx=%d,m=%.3f).txt", dir, Ntm, Nxm, Nt, Nx, m);
    xene_mass=fopen(filemisure,"w");
    control_file(xene_mass);
    
/*    sprintf(filemisure,"/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/%s/Bootstrap/misure_boot_spat(Ntm=%.3f,Nxm=%.3f,Nt=%d,Nx=%d,m=%.3f).txt", dir, Ntm, Nxm, Nt, Nx, m);
    xene_spat=fopen(filemisure,"w");
    control_file(xene_spat);

    sprintf(filemisure,"/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/%s/Bootstrap/misure_boot_temp(Ntm=%.3f,Nxm=%.3f,Nt=%d,Nx=%d,m=%.3f).txt", dir, Ntm, Nxm, Nt, Nx, m);
    xene_temp=fopen(filemisure,"w");
    control_file(xene_temp);*/

    sprintf(filemisure,"/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/%s/Bootstrap/misure_boot_density_NO_RELAX(Ntm=%.3f,Nxm=%.f,Nt=%d,Nx=%d,m=%.3f).txt", dir, Ntm, Nxm, Nt, Nx, m);
...
int main(){
    ...
    if(scelta==1 || scelta==2){
        ...
        printf("############################################################# \n");
        sprintf(valori,"/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/valori_cont.txt");  //leggo il file valori.txt
        f=fopen(valori,"r");
        control_file(f);
...
//caso in cui scelga di fare entrambe le simulazioni altrimenti la lista parte da NULL e non gli piace
    ...
    if (scelta == 0 || scelta == 2){
        ...
        sprintf(valori,"/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/valori_temp.txt");  //leggo il file valori.txt
        f=fopen(valori,"r");
        control_file(f);
        list *Nt=NULL;
        Nt = scan_file(f,Nt);         // lettura degli valori degli nt, con funzione ("listfunction.h")
        ...
        while(Nt!=NULL){
            ...
            char filemisure[500];
            Ntm = Nt->val * singolom;
            sprintf(filemisure, "/home/dario/Documents/UNI/Metodi/Modulo3/Scalar2D/Temperatura/misure_NO_RELAX(Ntm=%.3f,Nxm=%.3f,Nt=%.0f,Nx=%d,m=%.3f).txt", Ntm, Nxm,Nt->val, Nx, singolom); //it modifies each time the name of the file to etae created
            misure[p]=fopen(filemisure, "r");
            control_file(misure[p]);
