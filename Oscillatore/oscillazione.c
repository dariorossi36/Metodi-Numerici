...
double * measure(double obs[3]){
    ...
    obs[2] = obs[2]/Nlatt;	//media sul singolo path del campo e basta sommato su tutti i path
...
int main(){
    ...
    control_file(input);
    printf("2\n");
    misure = fopen("/home/dario/Documents/UNI/Metodi/Modulo2/Oscillatore/misure.txt","w" );
    printf("3\n");
    control_file(misure);
    printf("4\n");
    x= fscanf(input, "%f  %f  %d  %d  %d  %d  %d", &eta, &d_metro, &measures, &i_decorrel, &iflag, &i_term, &Nlatt);
    printf("5\n");
    ...
    double media[measures];

    //file per accettanza
    acc=fopen("/home/dario/Documents/UNI/Metodi/Modulo2/Oscillatore/accettanza.txt","w");
    control_file(acc);
    //TERMALIZZAZIONE
    ...
    for (int iter=0; iter<measures; iter++){
        ...
        obs[2] = measure(obs)[2];
        //printf("%lf  %lf  %d\n", obs[0], obs[1], iter);
        fprintf(misure,"%lf  %lf  %lf	%lf  %d\n", obs[0], obs[1], obs[2], media[iter], iter); //prendo misure a questa configurazione
    }
